...
def solution(name, yearning, photo):
    answer = []
    sum_answer = 0
    # yearning_dict = {name : yearning for i in range(len(name))}
    # name_dict = {key : value for key,value in enumerate(name)}
    # yearning_dict = {key : value for key, value in enumerate(yearning)}
    for i in photo:
        for j in i:
            if j in name:
                print("j가 name에 존재 : ", j)
                index = name.index(j)
                sum_answer += yearning[index]
            elif j not in name:
                continue
